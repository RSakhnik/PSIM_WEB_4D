...
getallDateFromX(x: number): Date {
    ...
    this.canvas.addEventListener('mouseup', (e: MouseEvent) => {
        ...
        lock_visualization(){
            const x = this.W - 25;
            const y = this.timelineHeight / 5 * 2;
            ...
            // Корпус замка. Рисуем прямо на canvas, чтобы не зависеть от внешнего lock.png.
            this.ctx.fillRect(x - 7, y - 1, 14, 10);

            // Дужка: закрытая при scale_lock=true, открытая при scale_lock=false.
            this.ctx.beginPath();
            this.ctx.moveTo(x - 5, y - 1);
            this.ctx.lineTo(x - 5, y - 5);
            this.ctx.arc(x, y - 5, 5, Math.PI, 0, false);
            if (this.scale_lock) {
                this.ctx.lineTo(x + 5, y - 1);
            }
            this.ctx.stroke();

            // Небольшая скважина для читаемости иконки.
            this.ctx.fillStyle = '#fff';
            this.ctx.beginPath();
            this.ctx.arc(x, y + 3, 1.2, 0, Math.PI * 2);
            this.ctx.fill();

            this.ctx.fillStyle = '#333';
            this.ctx.font = `10px "Segoe UI", Arial, sans-serif`;
            this.ctx.textAlign = 'center';
            this.ctx.textBaseline = 'middle';
            this.ctx.fillText('[Space]', x, this.timelineHeight / 5 * 4);
            this.ctx.restore();
        }
        update(){
    try {
