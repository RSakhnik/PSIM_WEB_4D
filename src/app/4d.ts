...
const notStartedToggle = document.createElement('button');
notStartedToggle.type = 'button';
notStartedToggle.setAttribute('aria-pressed', String(this.showNotStartedObjects));
notStartedToggle.style.cssText = 'display:flex; align-items:center; justify-content:center; width:30px; height:28px; padding:0; margin:0 4px; border:0; background:transparent; cursor:pointer; color:#4b5563; border-radius:4px;';

const updateNotStartedToggle = () => {
    notStartedToggle.setAttribute('aria-pressed', String(this.showNotStartedObjects));
    notStartedToggle.title = this.showNotStartedObjects
        ? 'Скрыть бледный фоновый слой (неначатые и непривязанные объекты)'
        : 'Показать бледный фоновый слой (неначатые и непривязанные объекты)';
    notStartedToggle.innerHTML = this.showNotStartedObjects
        ...
        : `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 3l18 18"/><path d="M10.6 10.6a2 2 0 0 0 2.8 2.8"/><path d="M9.9 4.2A10.9 10.9 0 0 1 12 4c6.5 0 10 8 10 8a18.4 18.4 0 0 1-2.1 3.2"/><path d="M6.6 6.6C3.7 8.4 2 12 2 12s3.5 8 10 8a10.8 10.8 0 0 0 5.4-1.4"/></svg>`;
};

updateNotStartedToggle();
notStartedToggle.addEventListener('click', (e) => {
    e.stopPropagation();
    this.showNotStartedObjects = !this.showNotStartedObjects;
    updateNotStartedToggle();
    this.applyColorsAsync();
});

// Вставляем фильтры и переключатель неначатых работ перед кнопкой закрытия
...
if (closeBtn) {
    ...
    headerDiv.insertBefore(notStartedToggle, closeBtn);
} else {
    headerDiv.appendChild(filter3dContainer);
    headerDiv.appendChild(filterCriticalContainer);
    headerDiv.appendChild(notStartedToggle);
}
    ...
    this.onDateChanged(date);
                ...
                model.eventDispatcher.addEventListener('modelPartLoadingProgress', () => {
                    ...
                    this.loadingTimeout = setTimeout(() => {
                        ...
                        this.backgroundElementsByPartCache = null;
                        this.backgroundCacheSignature = '';
                        this.backgroundVisibilityApplied = null;
...
private showNotStartedObjects: boolean = true;
...
filterIcon.style.cssText = 'margin-left:6px; display:inline-flex; align-items:center; cursor:pointer; opacity:0.6;';
    ...
    if (this.sortState.field) {
        ...
        items.sort((a, b) => {
            ...
            if (field === 'FactStartDate') {
                const dateA = this.getFactStartDate(a);
                const dateB = this.getFactStartDate(b);
                ...
                const dateA = this.getFactFinishDate(a);
                const dateB = this.getFactFinishDate(b);
                if (!dateA && !dateB) return 0;
                if (!dateA) return 1;
                if (!dateB) return -1;
                vA = dateA.getTime();
    ...
    items.forEach(item => {
        ...
        fact.textContent = factDate ? this.formatDateToString(factDate) : '';
        ...
        factFinish.textContent = factFinishDate ? this.formatDateToString(factFinishDate) : '';
...
if (this._dataInterval) clearInterval(this._dataInterval);
    ...
    private parseActualDate(value: unknown): Date | null {
    ...
    private getFactStartDate(item: WorkSchedule): Date | null {
        return this.parseActualDate(item.ActualStartDate);
    }

    private getFactFinishDate(item: WorkSchedule): Date | null {
        return this.parseActualDate(item.ActualFinishDate);
    }
    private FullSchedule: SheduleAndParts[]=[];
    private async db_query(){
        const DBs = ['dbRVS505', 'dbGEP', 'dbGPP_LEM', 'dbSOG'];
    ...
    // Все элементы тех model parts, которые встречаются в Work_ModelID.
    // Этот слой является базовым бледным контекстом 4D, даже если конкретный ElementID
    // не имеет привязки к работе.
    private backgroundElementsByPartCache: Record<string, string[]> | null = null;
    private backgroundCacheSignature: string = '';
    private backgroundVisibilityApplied: boolean | null = null;

    // Здесь храним только элементы, поверх базового фона окрашенные статусным цветом.
    // На следующем пересчёте их достаточно вернуть в базовый ghost-слой, не перекрашивая
    // заново всю модель.
...
private ensureBackgroundElementsCache(model: any, visibleParts: any[]): Record<string, string[]> {
    const targetPartIds = new Set(
        (this.ElementIds ?? [])
            .map(item => this.normalizeModelId(item.ModelPartID))
            .filter(Boolean)
    );

    let allParts: any[] = [];
    try {
        allParts = model.getAllModelParts?.() ?? [];
    } catch (_) {}

    const loadedPartIds = allParts
        .map(part => this.normalizeModelId(part?.id ?? part?.modelPartId))
        .filter(Boolean)
        .sort();
    const signature = `${Array.from(targetPartIds).sort().join('|')}::${loadedPartIds.join('|')}`;

    if (this.backgroundElementsByPartCache && this.backgroundCacheSignature === signature) {
        return this.backgroundElementsByPartCache;
    }

    const result: Record<string, string[]> = {};
    const visibleByPart: Record<string, string[]> = {};

    // Fallback: getVisibleElements() уже даёт реальные elementIds по modelPartId.
    for (const part of visibleParts ?? []) {
        const partId = this.normalizeModelId(part?.modelPartId ?? part?.id);
        if (!partId || !targetPartIds.has(partId) || !Array.isArray(part?.elementIds)) continue;
        visibleByPart[partId] = part.elementIds
            .map((id: unknown) => this.normalizeModelId(id))
            .filter(Boolean);
    }

    // Основной путь такой же, как в расширении коллизий: берём все entities части модели.
    for (const part of allParts) {
        const partId = this.normalizeModelId(part?.id ?? part?.modelPartId);
        if (!partId || !targetPartIds.has(partId)) continue;

        const ids = new Set<string>();
        const entities = part?.elementTree?._entities
            ?? part?._modelEntityTreeAccess?._entities
            ?? part?.elementTree?.entities;

        try {
            if (entities && typeof entities.forEach === 'function') {
                entities.forEach((value: any, key: any) => {
                    const candidate = typeof key === 'string'
                        ? key
                        : (value?.id ?? value?.elementId);
                    const id = this.normalizeModelId(candidate);
                    if (id) ids.add(id);
                });
            } else if (Array.isArray(entities)) {
                for (const entity of entities) {
                    const id = this.normalizeModelId(entity?.id ?? entity?.elementId);
                    if (id) ids.add(id);
                }
            }
        } catch (e) {
            console.warn('[4D] Не удалось перечислить элементы model part', partId, e);
        }

        // На случай отличий внутренней структуры SDK используем публичный список видимых элементов.
        if (ids.size === 0 && visibleByPart[partId]) {
            for (const id of visibleByPart[partId]) ids.add(id);
        }

        if (ids.size > 0) result[partId] = Array.from(ids);
    }

    // Если getAllModelParts() не дал часть, но она присутствует в getVisibleElements().
    for (const [partId, ids] of Object.entries(visibleByPart)) {
        if (!result[partId] && ids.length) result[partId] = Array.from(new Set(ids));
    }

    this.backgroundElementsByPartCache = result;
    this.backgroundCacheSignature = signature;
    // Новый/перезагруженный model part нужно обязательно покрасить базовым слоем.
    this.backgroundVisibilityApplied = null;

    const total = Object.values(result).reduce((sum, ids) => sum + ids.length, 0);
    console.log(`[4D] Базовый ghost-слой: ${Object.keys(result).length} model parts, ${total} элементов`);
...
private _doApplyColors(model: any): void {
    ...
    const baseColor = this.showNotStartedObjects
        ? { r: 235, g: 235, b: 235, a: 0.08 }
        : { r: 255, g: 255, b: 255, a: 0.0 };

    const visibleParts = model.getVisibleElements?.() ?? [];
    if (!visibleParts?.length) return;

    const backgroundByPart = this.ensureBackgroundElementsCache(model, visibleParts);

    // Возвращаем прошлые зелёные/жёлтые/красные элементы обратно в текущий базовый слой.
    // Это дешёво по сравнению с полной перекраской всех model parts на каждой смене даты.
    for (const [partId, ids] of Object.entries(this.lastPainted)) {
        if (!ids?.length) continue;
        setChunked(ids, baseColor.r, baseColor.g, baseColor.b, baseColor.a, partId);
    }
    this.lastPainted = {};

    // Полностью перекрашиваем фон только при первом запуске, загрузке новой части модели
    // или переключении глазика. Таким образом ВСЕ объекты частей из Work_ModelID получают
    // бледный фон, включая элементы без собственной строки в Work_ModelID.
    if (this.backgroundVisibilityApplied !== this.showNotStartedObjects) {
        for (const [partId, ids] of Object.entries(backgroundByPart)) {
            if (!ids?.length) continue;
            setChunked(ids, baseColor.r, baseColor.g, baseColor.b, baseColor.a, partId);
        }
        this.backgroundVisibilityApplied = this.showNotStartedObjects;
    }
    ...
    if (!this.leafToPartMapCache) {
        ...
        for (const part of visibleParts) {
            ...
            const partId = this.normalizeModelId(part.modelPartId);
            for (const elem of part.elementIds) {
                this.leafToPartMapCache[this.normalizeModelId(elem)] = partId;
            }
        }
    }

    const greens  = new Set(this.GSortedElementIds.map(e => this.normalizeModelId(e.ElementID)));
    const yellows = new Set(this.YSortedElementIds.map(e => this.normalizeModelId(e.ElementID)));
    const chosen  = new Set((this.ChoosenOne ?? []).map((e: string) => this.normalizeModelId(e)));

    type Buckets = { greens: string[], yellows: string[], choosen: string[] };
    const partColors: Record<string, Buckets> = {};

    // Привязанные к работам элементы накладываются поверх общего ghost-фона.
    // Неначатые отдельно красить больше не нужно: они уже находятся в базовом слое.
    for (const [parentIdRaw, leaves] of Object.entries(this.elementTree)) {
        const parentId = this.normalizeModelId(parentIdRaw);
        const bucket: keyof Buckets | null =
            chosen.has(parentId)  ? 'choosen' :
            greens.has(parentId)  ? 'greens'  :
            yellows.has(parentId) ? 'yellows' : null;

        if (!bucket) continue;

        for (const leaf of leaves) {
            const leafId = this.normalizeModelId(leaf);
            const partId = this.normalizeModelId(this.leafToPartMapCache[leafId]);
            if (!partId) continue;
            if (!partColors[partId]) partColors[partId] = { greens:[], yellows:[], choosen:[] };
            partColors[partId][bucket].push(leafId);
        }
    }
    ...
    for (const [partId, colors] of Object.entries(partColors)) {
        ...
        const paint = (ids: string[], r: number, g: number, b: number, a: number) => {
            ...
            setChunked(uniqueIds, r, g, b, a, partId);
            this.lastPainted[partId].push(...uniqueIds);
        };
...
private clearLastPainted(model: any) {
    const CHUNK = 2;
    const allPainted: Record<string, Set<string>> = {};

    const collect = (source: Record<string, string[]> | null | undefined) => {
        if (!source) return;
        for (const [partId, ids] of Object.entries(source)) {
            if (!allPainted[partId]) allPainted[partId] = new Set<string>();
            for (const id of ids ?? []) allPainted[partId].add(id);
        }
    };

    collect(this.backgroundElementsByPartCache);
    collect(this.lastPainted);

    for (const [partId, idSet] of Object.entries(allPainted)) {
    ...
    this.lastPainted = {};
    // После закрытия панель очистила фон; при следующем открытии его надо нанести заново.
    this.backgroundVisibilityApplied = null;
}
